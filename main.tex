...
\begin{document}
    ...
    \vspace{-2cm}
        ...
        \AND%
        \kern 2.0 pt%
        \mbox{\hrefWithoutArrow{tel:+1 7745190324}{7745190324}}%
        \kern 2.0 pt%
        \AND%
        \kern 2.0 pt%
    ...
    \section{Skills}
        ...
        \begin{onecolentry}
            \textbf{LLM \& GenAI:} OpenAI API, Claude API, Mistral, LangChain, LangGraph, Agentic AI, RAG, Pinecone, Fine-Tuning, Prompt Engineering \\
            \textbf{ML \& NLP:} PyTorch, TensorFlow, Keras, Hugging Face, Sentence-Transformers, BERTScore, Scikit-learn, XGBoost, spaCy, NLTK, Gensim \\
            \textbf{Cloud \& MLOps:} AWS (Bedrock, SageMaker, Lambda, Step Functions, Textract, Comprehend, CloudFormation), GCP, Vercel, W\&B, Docker, Kubernetes, CI/CD \\
            \textbf{Languages:} Python, SQL, TypeScript, R, C/C++ \\
            \textbf{Data \& Tools:} Pandas, NumPy, Pydantic, FastAPI, Streamlit, Playwright, Pillow, PostgreSQL, MongoDB, Git
        \end{onecolentry}
    ...
    \section{Experience}
        ...
        \begin{onecolentry}
            \begin{highlights}
                ...
                \item Constructed RAG fallback system with \textbf{Titan embeddings}, in-memory vector store, and \textbf{cosine similarity retrieval} to resolve ambiguities across 50+ Form I-129 fields, achieving 95-99\% extraction accuracy.
                \item Integrated LLM validation layer using \textbf{OpenAI}, \textbf{Pydantic} schema enforcement, and \textbf{Textract}-based KB construction, cutting manual verification effort by 70-90\%.
            \end{highlights}
        \end{onecolentry}
        ...
        \begin{onecolentry}
            \begin{highlights}
                ...
                \item Attained 0.9417 cosine similarity with Amazon's proprietary summaries using \textbf{scikit-learn} cosine similarity, \textbf{iterative prompt refinement}, and \textbf{BERTScore} validation on \textbf{Playwright}-scraped review data.
            \end{highlights}
        \end{onecolentry}
        ...
        \begin{onecolentry}
            \begin{highlights}
                \item Deployed end-to-end NER pipeline using \textbf{AWS Textract}, \textbf{SageMaker Ground Truth} (custom annotation template), and \textbf{Comprehend Custom Entity Recognition} to extract entities from native PDF/Word documents, achieving 99\% accuracy.
    ...
    \section{Projects}
        ...
        \begin{twocolentry}{
            \hrefWithoutArrow{https://huggingface.co/spaces/pradyten/pdf-extractor}{Live Demo}
        }
        ...
        \begin{onecolentry}
            \begin{highlights}
                \item Built vision-based PDF extraction system using \textbf{OpenAI Vision API} (GPT-4.1), \textbf{pypdfium2}, and \textbf{Pillow} with adaptive DPI rendering, supporting 12 document types (I-129, passports, resumes) with JSON schema enforcement, deployed on \textbf{Hugging Face Spaces}.
            \end{highlights}
        \end{onecolentry}
        ...
        \begin{twocolentry}{
            \hrefWithoutArrow{https://legal-ai-hazel-xi.vercel.app/}{Live Demo}
        }
            \textbf{Legal AI Research Assistant}
        \end{twocolentry}

        \vspace{0.10 cm}
        \begin{onecolentry}
            \begin{highlights}
                \item Created full-stack RAG application using \textbf{LangGraph} (6-node pipeline), \textbf{FastAPI}, \textbf{GPT-4o}, and \textbf{Pinecone} for legal case research, with dual-layer confidence scoring (60\% retrieval + 40\% LLM self-assessment) and \textbf{Mistral} fallback, deployed on \textbf{Vercel}/\textbf{Railway}.
            \end{highlights}
        \end{onecolentry}
        ...
        \begin{onecolentry}
            \begin{highlights}
                \item Co-authored paper (accepted at \textbf{ICFAiSE 2025}) achieving R\textsuperscript{2}=0.98 using \textbf{LSTM} (\textbf{TensorFlow/Keras}, \textbf{PyTorch}) for turbidity prediction through 2-year time-series analysis of Indian lakes, outperforming ARIMA and linear regression baselines.
            \end{highlights}
        \end{onecolentry}
